...
export default function StoreShell({
  ...
  const decodedStorecode = decodeURIComponent(storecode);
        ...
        <aside className="hidden w-[272px] shrink-0 border-r border-slate-200 bg-slate-950 text-white lg:block">
          <div className="sticky top-1/2 flex max-h-[calc(100vh-2.5rem)] -translate-y-1/2 flex-col gap-6 overflow-y-auto px-5 py-6">
            <div className="rounded-[28px] border border-white/10 bg-white/5 px-4 py-4">
              ...
              <div className="mt-3 inline-flex rounded-full border border-sky-300/20 bg-sky-400/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-sky-100">
                {decodedStorecode}
              </div>
            ...
            <nav className="space-y-2">
              {NAV_ITEMS.map((item) => {
                const href = `/${lang}/${storecode}${item.href}`;
            ...
            <div className="px-4 py-4">
              <div className="rounded-[28px] bg-slate-950 px-4 py-4 text-white shadow-[0_20px_45px_-28px_rgba(15,23,42,0.8)]">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="inline-flex shrink-0 rounded-full border border-sky-300/20 bg-sky-400/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.14em] text-sky-100">
                    {decodedStorecode}
                  </div>
                </div>

                <div className="mt-3 text-xs leading-5 text-slate-300">
                  현재 가맹점 범위에서 주문, 청산, 일별마감, 회원관리, 설정 화면으로 이동할 수 있습니다.
                </div>
