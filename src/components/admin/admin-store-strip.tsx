    ...
    <div className={`pointer-events-none fixed inset-x-0 z-40 ${stickyTopClassName}`}>
      <div className="mx-auto w-full max-w-[1880px] px-4 sm:px-5 lg:px-8">
        <div className="pointer-events-auto lg:ml-[272px]">
          <aside className="console-panel w-full overflow-hidden rounded-[24px] border border-sky-100/80 bg-[linear-gradient(180deg,rgba(249,252,255,0.98),rgba(240,247,255,0.94))] shadow-[0_18px_48px_-36px_rgba(14,165,233,0.38)] backdrop-blur">
            <div className="max-h-[calc(100vh-12.75rem)] overflow-y-auto overscroll-y-contain lg:max-h-none lg:overflow-visible">
            <div className="border-b border-sky-100/80 px-3.5 py-3 sm:px-4 sm:py-3.5">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="space-y-1.5">
                  <div className="console-mono text-[11px] font-medium uppercase tracking-[0.16em] text-slate-500">
                    Stores
                  </div>
                  <h2 className="console-display text-[1.35rem] font-semibold tracking-[-0.05em] text-slate-950 sm:text-[1.5rem]">
                    {title}
                  </h2>
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  {selectedStore ? (
                    <div className="hidden min-w-[220px] items-center gap-2.5 rounded-[20px] border border-slate-200/80 bg-white/85 px-3 py-2 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.9)] lg:flex">
                      <StoreLogo
                        src={getStoreLogoSrc(selectedStore)}
                        alt={getStoreDisplayName(selectedStore)}
                        className="h-10 w-10 shrink-0 rounded-2xl border border-sky-100 bg-sky-50/70"
                      />
                  ...
                  <button
                    ...
                    className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[11px] font-semibold text-slate-600 transition hover:border-sky-200 hover:bg-sky-50 hover:text-sky-700"
                  >
              ...
              {isExpanded ? (
                <div className="mt-3">
                  <input
                    ...
                    className="h-10 w-full rounded-[18px] border border-slate-200 bg-white px-3.5 text-[14px] text-slate-900 outline-none transition focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-100"
                  />
                </div>
              ) : null}
              ...
              <div className="px-3.5 py-3 sm:px-4 sm:py-3.5">
                ...
                {!loading && !combinedError && filteredStores.length > 0 ? (
                  <div className="overflow-x-auto overscroll-x-contain">
                    <div className="flex min-w-full gap-2.5 pb-1">
                      {allowAllStores ? (
                        <article
                          className={`w-[184px] shrink-0 rounded-[20px] border p-3 transition sm:w-[210px] ${
                        ...
                        >
                          ...
                          >
                            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border text-[10px] font-semibold uppercase tracking-[0.14em] ${
                              normalizedSelectedStorecode === normalizeText(allStoresValue)
                      ...
                      {filteredStores.map((store) => {
                        ...
                        return (
                          <article
                            ...
                            className={`w-[192px] shrink-0 rounded-[20px] border p-3 transition sm:w-[220px] ${
                              selected
                                ? "border-transparent bg-[linear-gradient(135deg,#0f172a_0%,#1d4ed8_54%,#0f766e_100%)] text-white shadow-[0_22px_42px_-24px_rgba(37,99,235,0.52)]"
                                : "border-slate-200/90 bg-white/90 text-slate-900 hover:border-sky-200 hover:bg-[linear-gradient(180deg,rgba(248,252,255,0.98),rgba(240,249,255,0.98))]"
                            }`}
